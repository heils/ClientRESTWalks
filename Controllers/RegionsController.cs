...
namespace Client.Controllers
{
    public class RegionsController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var httpResponseMessage = await client.GetAsync("https://app-walks-canada-dev-001.azurewebsites.net/api/regions");
        ...
        public async Task<IActionResult> Add(CreateRegionDto regionDto)
        {
            ...
            var httpRequestMessage = new HttpRequestMessage()
            {
                ...
                RequestUri = new Uri("https://app-walks-canada-dev-001.azurewebsites.net/api/regions"),
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            var token = HttpContext.Session.GetString("AccessToken");
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var httpResponseMessage = await client.GetAsync($"https://app-walks-canada-dev-001.azurewebsites.net/api/regions/{id.ToString()}");
            if (httpResponseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> Edit(RegionDto regionDto)
        {
            ...
            var httpRequestMessage = new HttpRequestMessage()
            {
                ...
                RequestUri = new Uri($"https://app-walks-canada-dev-001.azurewebsites.net/api/regions/{regionDto.Id.ToString()}"),
                Content = new StringContent(JsonSerializer.Serialize(regionDto), Encoding.UTF8, "application/json")
            };
            var httpResponseMessage = await client.SendAsync(httpRequestMessage);
            httpResponseMessage.EnsureSuccessStatusCode();
        ...
        public async Task<IActionResult> Delete(RegionDto regionDto)
        {
            ...
            var token = HttpContext.Session.GetString("AccessToken");
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var httResponseMessage = await client.DeleteAsync($"https://app-walks-canada-dev-001.azurewebsites.net/api/regions/{regionDto.Id.ToString()}");
